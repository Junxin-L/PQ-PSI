...
party_mean_total() {
    local report_file="$1"
    local party="$2"
    perl -ne 'if (/^### '"$party"' \(mean total ([0-9.]+) ms\)/) { print $1; exit }' "$report_file"
}
...

{
    ...
    printf -- "- \`Party0 PSI Avg (ms)\`: internal mean total for party0\n"
    ...
    printf "## Params\n\n"
    printf "| Set Size | eps | w | m | lambda_real |\n"
    printf "| --- | ---: | ---: | ---: | ---: |\n"
} > "$OUTPUT_FILE"
...
cleanup_rows() {
    rm -f "$param_rows_file" "$perf_rows_file"
}
...
for i in "${!sizes[@]}"; do
    ...
    party0_avg="$(party_mean_total "$report_file" "party0")"
    ...
    {
        printf "| 2^%s | %s | %s | %s | %s |\n" "$power" "$rb_eps" "$rb_w" "$rb_m" "$rb_lambda_real"
    } >> "$param_rows_file"
    {
        printf "| 2^%s | %s | %s |\n" "$power" "$party0_avg" "$total_comm_avg"
    } >> "$perf_rows_file"
done
...
printf "\n" >> "$OUTPUT_FILE"
printf "## Performance\n\n" >> "$OUTPUT_FILE"
printf "| Set Size | Party0 PSI Avg (ms) | Total Comm Avg (KB) |\n" >> "$OUTPUT_FILE"
printf "| --- | ---: | ---: |\n" >> "$OUTPUT_FILE"
...
for i in "${!sizes[@]}"; do
    ...
    power="${powers[$i]}"
    report_file="$DETAIL_ROOT/${BENCH_STEM}_set-${size}_warm-${WARMUPS}_rounds-${ROUNDS}_delay-${SAFE_DELAY}_bw-${SAFE_BW}_${RB_TAG}.md"
    runtime_avg="$(summary_value "$report_file" "runtime_mean_ms")"
    total_comm_avg="$(summary_value "$report_file" "total_communication_kb_mean")"
    party0_avg="$(party_mean_total "$report_file" "party0")"
    party1_avg="$(party_mean_total "$report_file" "party1")"

    {
        printf "## Set Size 2^%s\n\n" "$power"
        printf "| Run | Runtime (ms) | Party0 Total (ms) | Party1 Total (ms) | Total Comm (KB) |\n"
        printf "| --- | ---: | ---: | ---: | ---: |\n"
        printf "| Avg | %s | %s | %s | %s |\n" "$runtime_avg" "$party0_avg" "$party1_avg" "$total_comm_avg"
        append_round_rows "$report_file" | awk -F'|' '
            {
                round = $2
                runtime = $3
                party0 = $4
                ...
                printf "| %s | %s | %s | %s | %s |\n", round, runtime, party0, party1, comm
            }
        '
        printf "\n"
        if [[ "$DETAIL_MODE" != "none" ]]; then
