...
docker run --rm --platform linux/amd64 \
    ...
    bash -lc "apt-get update >/tmp/apt-update.log 2>&1 && \
        apt-get install -y --no-install-recommends build-essential cmake git libboost-system-dev libboost-thread-dev libgmp-dev libsodium-dev >/tmp/apt-install.log 2>&1 && \
        ...
        cmake --build ${CONTAINER_BUILD_DIR} --target pqpsi_rbokvs_bench pqpsi_party_bench rbokvs_g_check rbokvs_pqpsi_test pqpsi_tests construction_permutation_bench --parallel 1 >${CONTAINER_BUILD_LOG} 2>&1"

echo "Built pqpsi_rbokvs_bench, pqpsi_party_bench, rbokvs_g_check, rbokvs_pqpsi_test, pqpsi_tests, and construction_permutation_bench into $BUILD_DIR"
